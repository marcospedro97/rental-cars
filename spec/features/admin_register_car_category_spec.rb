# frozen_string_literal: true

require 'rails_helper'

feature 'Admin register car category' do
  scenario 'successfully' do
    # Arrange
    user = create(:user)
    login_as user, scope: :user
    # Act
    visit root_path
    click_on 'Categorias de Carros'
    click_on 'Registrar nova categoria'
    fill_in 'Nome', with: 'C'
    fill_in 'Diária Padrão', with: '299'
    fill_in 'Seguro do Carro', with: '2990'
    fill_in 'Seguro para Terceiros', with: '3990'
    click_on 'Enviar'
    # Assert
    expect(page).to have_content 'Categoria C'
    expect(page).to have_content 'Diária Padrão: 299'
    expect(page).to have_content 'Seguro do Carro: 2990'
    expect(page).to have_content 'Seguro para Terceiros: 3990'
  end

  scenario 'and validates all fields' do
    # Arrange
    user = User.create!(email: 'teste@teste.com', password: '123456')
    login_as user, scope: :user
    # Act
    visit root_path
    click_on 'Categorias de Carros'
    click_on 'Registrar nova categoria'
    fill_in 'Nome', with: ''
    fill_in 'Diária Padrão', with: 'Duzentos'
    fill_in 'Seguro do Carro', with: 'Vinte e nove'
    fill_in 'Seguro para Terceiros', with: 'Trezentos'
    click_on 'Enviar'
    # Assert
    expect(page).to have_content 'Nome não pode ficar em branco'
    expect(page).to have_content 'Diária Padrão deve ser um valor numérico'
    expect(page).to have_content 'Seguro do Carro deve ser um valor numérico'
    expect(page).to have_content 'Seguro para Terceiros deve ser um valor numérico'
  end
  scenario 'must be authenticated via route' do
    # Act
    visit new_car_category_path
    # Assert
    expect(current_path).to eq(new_user_session_path)
  end
end
